...
def generate_release_candidates(
    ...
    If max_release > 0, at most that many components are randomly
    selected for release to bound per-step evaluations.
    """
    if len(solution) <= 1:
        return []
    components = list(solution)
    if 0 < max_release < len(components):
        if rng is None:
            rng = np.random.default_rng()
        components = rng.choice(components, max_release, replace=False).tolist()
    return [solution - {r} for r in components]
...
def generate_all_candidates(
    ...
    if include_release:
        candidates.extend(generate_release_candidates(solution, max_release, rng))
    return candidates
